...
export default function ExecutiveMinutesView() {
  const { id } = useParams();
  ...
  const fetchMinutes = async () => {
    setLoading(true);
    try {
      const response = await api.get(`/api/executive/minutes/${id}`, { headers });
  ...
  try {
    ...
    // Build members present HTML
    let membersPresentHTML = sortedExecutives.map((member, idx) => {
    ...
    // Build in-attendance HTML
    let inAttendanceHTML = '';
    ...
    // Build absent members HTML
    let absentMembersHTML = '';
    if (minutes?.absentMembers?.length > 0) {
      absentMembersHTML = `
    ...
    // Build absent with apology HTML
    let absentApologyHTML = '';
    const excusedMembers = minutes?.absentMembers?.filter(m => m.excused) || [];
    if (excusedMembers.length > 0) {
      absentApologyHTML = `
        <h2>Absent with Apology</h2>
        <div class="members-list">
          ${excusedMembers.map((member, idx) => `<div class="member-item">${idx + 1}. ${member.fullName}</div>`).join('')}
        </div>
      `;
    }
    ...
    // Build preliminaries HTML
    let preliminariesHTML = '';
    if (minutes?.preliminaries) {
      preliminariesHTML = `
    ...
    let sectionsHTML = '';
    if (minutes?.sections) {
      sectionsHTML = minutes.sections.map(section => `
    ...
    // Build adjournment HTML
    let adjournmentHTML = '';
    if (minutes?.adjournment) {
      adjournmentHTML = `
      ...
      return (
        <div className="create-minutes-page">
          ...
          <div className="loading-container">
            ...
            <p>Opening minutes for</p> <p2><strong>{minutes?.title || 'ZUCA'}</strong>...</p2>
          </div>
          <style>{`
    ...
    return (
      <div className="minutes-view-page">
        <div className="error-container">
          ...
          <button onClick={() => navigate('/executive/minutes')} className="back-btn">Back to Minutes</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="minutes-view-page">
      <div className="action-bar no-print">
        <button className="back-btn" onClick={() => navigate('/executive/minutes')}>
