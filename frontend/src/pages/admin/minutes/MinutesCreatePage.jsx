  ...
  <div className="preview-section">
    ...
    {sheetData.absentMembers.filter(m => m.excused).map((member, idx) => (
      <div key={idx}>{idx + 1}. {member.fullName}</div>
         ...
         {formData.aob.filter(a => a.title.trim()).length > 0 && (<div className="preview-section"><h2>MIN {String(formData.sections.filter(s => s.title.trim()).length + 1).padStart(2, '0')}/{String(new Date().getMonth() + 1).padStart(2, '0')}: AOB</h2>{formData.aob.filter(a => a.title.trim()).map((item, idx) => (<div key={idx}><strong>{item.title}</strong>{item.content && <p>{item.content}</p>}</div>))}</div>)}
         {formData.adjournment && (<div className="preview-section"><h2>MIN {String(formData.sections.filter(s => s.title.trim()).length + (formData.aob.filter(a => a.title.trim()).length > 0 ? 2 : 1)).padStart(2, '0')}/{String(new Date().getMonth() + 1).padStart(2, '0')}: ADJOURNMENT</h2><p>{formData.adjournment}</p></div>)}
